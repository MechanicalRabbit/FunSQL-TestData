...
const DEMOZIP = "https://physionet.org/content/eicu-crd-demo/get-zip/2.0.1/"
const BUILDDIR = joinpath(tempdir(), "eicu-crd-build")

function download_demo()
    mkpath(BUILDDIR)
    @info "Using directory: $BUILDDIR"
    zip_path = joinpath(BUILDDIR, "eicu-crd-demo-2.0.1.zip")
    if !isfile(zip_path)
...
@info "Starting eicu-crd database creation..."
if length(ARGS) > 0
    zip_path = ARGS[1]
    if !isfile(zip_path)
